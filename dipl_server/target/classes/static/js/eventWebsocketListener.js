...
function connect() {
    var socket = new SockJS('http://77.47.130.174:9090/broadcast/events/get/all');
...
function showEvents(message) {
    ...
    var date = new Date(message.creationDate);
    message.creationDate = date;
    var dt = dynamicTable.config('data-table', 
                                 ['id', 'terminal','staffMember', 'eventType','creationDate'], 
                                 ['Event ID', 'Terminal','Staff Member','Event type','Creation time'], //set to null for field names instead of custom header names
