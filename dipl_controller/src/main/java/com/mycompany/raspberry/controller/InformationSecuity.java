...
public class InformationSecuity {
    ...
    public String decodeSecuredData(SecuredData d) throws Exception{
        byte[] decoded = Base64.getDecoder().decode(EncryptionModule.decrypt(Base64.getDecoder().decode(d.key)));
        SecretKey originalKey = new SecretKeySpec(decoded, 0, decoded.length, "AES");
        return EncryptionModule.decryptSymmetric(d.data, originalKey);
